...
def execute_rebalancing(holdings, cash, target_portfolio, prices, logs):
    ...
    for t in [t for t in holdings if holdings[t] > 0 and t not in target_portfolio]:
        ...
        if shares > 0 and pd.notna(price):
            ...
            logs.append({"type": "TRANSACTION", "action": "SELL_ALL", "ticker": t, "shares": shares, "price": price, "amount": base_proceeds, "fee": base_proceeds * SELL_TAX_RATE})
    ...
    for ticker, weight in target_portfolio.items():
        ...
        if delta > 0:
            ...
            if shares > 0 and holdings.get(ticker, 0) >= shares:
                ...
                cash += base_proceeds * (1 - SELL_TAX_RATE)
                logs.append({"type": "TRANSACTION", "action": "SELL_ADJUST", "ticker": ticker, "shares": shares, "price": price, "amount": base_proceeds, "fee": base_proceeds * SELL_TAX_RATE})
