...
def run_backtest(params: dict):
    ...
    interval = params["interval"]  # interval 값 추출
    ...
    # [수정] Pandas 경고 메시지를 해결하기 위해 'M'을 'ME'로 변경
    if interval.endswith("M"):
        interval = interval.replace("M", "ME")
    ...
    evaluation_dates = evaluation_dates[
        (evaluation_dates >= sim_start_date) & (evaluation_dates <= sim_end_date)
    ]
            ...
            if i == 0:
                    ...
                    if price is not None and price > 0:
                        ...
                        if shares > 0:
                            base_cost, commission = (
                                shares * price,
                                (shares * price) * BUY_COMMISSION_RATE,
                            )
                ...
                if strategy == "default" and no_rebalance:
                    ...
                    holdings, cash = execute_rebalancing(
                        holdings.copy(), cash, target_portfolio, current_prices, logs
                    )
